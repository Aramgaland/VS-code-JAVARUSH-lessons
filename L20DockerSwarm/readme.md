...
Как обновить pip внутри твоего venv?
Ты уже находишься в окружении (venvL20), поэтому просто выполни:
...
\\\\\\ есть еще команда когда надо новый поетри инициализировать   -   
 poetry new my_new_project    и поетри создаст проект с директорией и со своей структурой на подобии шаблона
...
\\\\\\\\\\\\\\\\\ Нужна некоторая подготовка перед выполнением  POETRY INSTALL \\\\\\\\\\\\\\\
...
ты глобально говоришь Poetry: «все новые проекты должны создавать виртуальное окружение прямо внутри папки проекта» (обычно это .venv).
То есть теперь, когда ты будешь делать poetry install в новом проекте:
Виртуальное окружение создастся не где-то в глобальном Poetry-кэше, а прямо в папке проекта.
Это удобно, чтобы не держать все окружения в одном месте и легко переносить проект.
Если позже захочешь вернуть стандартное поведение (создавать окружения в общем хранилище Poetry), можно выполнить:

poetry config --unset virtualenvs.in-project

И глобальная настройка снова будет «по умолчанию».


poetry install     ////// установим все зависимости (аналог pip install -r)
...
poetry add --dev pytest    \\\\\  если хотим установить зависимость для разработки в файле pyproject.toml   появятся строки tool.portry.dev.dependences

poetry update requests   //// обновить зависимость
...
<!-- если бы у нас был файл  test.main.py   то он бы высветился  и выполнился  -->
poetry run pytest
...
<!-- можно обновлять не все зависимости, а только определенные -->
poetry update --only dev     \\\ тип только обновить окрудение для разработки develop 

poetry add --group link ruff \\\\ создали еще  подгруппу линк
poetry add --dev mypy

poetry show   ///// показать зависимости

poetry install --only main   //// установить только главные зависимости

pip install --with dev   \\\\ ставим в окружение зависимости из определенной группы

poetry show --tree ///// показывает дерево  зависимосте.. супер интересно и познавательно


<!-- есть что то на подобии poytry  - uv  -->

надо поискать какой конмандой это устанавливается на винде, так кк это новый инструмент\\ он просто работает быстрее поетри

 uv pip install requests   \\\\\ uv так же установит  requests как с pip   
 ...
 ////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////
///////////////// DOCKER SWARM   L20      /////  Docker Swarm — это оркестратор контейнеров
Он позволяет запускать приложение не на одном компьютере, а на кластере из нескольких серверов, как единое целое.

Swarm = несколько машин объединены в один кластер
Контейнеры распределяются автоматически по этим машинам
Если один сервер умирает → контейнеры перезапускаются на другом
Можно масштабировать сервисы: 1 → 5 → 20 копий
...
Swarm вводится, когда у тебя:

приложение должно работать 24/7
приложение должно выдерживать нагрузку
приложение должно работать на нескольких серверах
нужен автоматический перезапуск сервисов
нужно масштабирование (replicas)
То есть он нужен уже на этапе деплоя, а не разработки.

docker swarm init      //// инициализиуем сварм

       \\\\\ создаем некий сервис который будет пробрасывать  внешний порт 9090 на внутренний 8800

docker servise scale my-web=5     \\\\ некое масштабирование

<!-- последовательность действий словами -->

сбилдили конейнер
сделали инит сварма
сощдали сервис myweb
потом через scale размножили его на 5

потом собираем контейнеры со след версией
docker servise update --image my-cwarm-app:2.0 --env-add APP_VERSION+2.0 my-web

docker stuck deploy -c docker-compose.yml myappp

docker stuck rm myapp ///если удалить весь стек

docker swarm leave --force .... чтобы покинуть сам сварм


сварм это такой же балансировщил или распределительл как и nginx


<!-- немного с визуализацией -->

docker volume create portainer_data      \\\\\\\создадим специальный том как некий сервис \\\ ит ам прям много параметров прописывается

Если ты хочешь использовать этот том как "сервис" (для Portainer)
Тогда нужно не в команду добавлять, а запускать контейнер Portainer с привязкой этого тома.
Вот правильный запуск Portainer:
...
docker run -d \
  ...
  ошибка понятна. В Swarm нельзя использовать build: напрямую в docker stack deploy. Swarm требует готовый образ, который уже существует в локальном Docker или в репозитории.
Вот что происходит:
Ignoring unsupported options: build — Swarm игнорирует build: в compose, потому что stack deploy не умеет собирать образы.
invalid image reference for service web: no image specified — потому что Swarm ожидает image: <имя_образа>, а его нет.

Собрать образ вручную и указать его
docker build -t exampleproject-web .


И в docker-compose.yml оставить только:

services:
  web:
    image: exampleproject-web
    ports:
      - "8000:8000"
    depends_on:
      - db
    networks:
      ...
      \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
     твой Docker пока не работает в режиме Swarm
docker swarm init              Инициализировать Swarm на локальном компьютере:


      docker stack deploy -c docker-compose.yml exampleproject

Посмотреть существующие сети Swarm:
docker network ls

Удалить старую сеть, если она не нужна:
docker network rm exampleproject_app-net



РАЗОБРАТЬСЯ КАК РАБОТАТЬ С PORTAINER IO !!!!!!!!!

 он нужен чтобы с графическим интерфейсом управлять контейнерами и управлять свармом
 ...
 \\\\\\\\\\  Мониторинг в Docker  \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
  ...
  docker stats  общая статитстика

  docker stats -a  (--all) стата по всем контейнерам включая остановленные

  docker stats -f (--format) задает формат вывода 

  <container_id>
 <container_name>  выдает статистику ток указанного контейнера